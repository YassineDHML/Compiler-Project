...
from parser import Parser, Decl, Assign, BinOp, Num, Id, Str, IfStmt
...
def node_to_dict(node):
    ...
    if isinstance(node, IfStmt):
        return {
            ...
            'body': [node_to_dict(n) for n in node.body],
            'else_body': [node_to_dict(n) for n in node.else_body] if node.else_body else None
        }
    return {}
...
def run():
    ...
    try:
        ...
        result['stages']['parser'] = {
            'ok': True,
            'ast': [node_to_dict(n) for n in ast],
            'repr': repr(ast)
        }
    except SyntaxError as e:
    ...
    try:
        ...
        inferred_types = []
        for node in ast:
            inferred_types.append(analyzer.analyze(node))
            
        inferred = inferred_types[-1] if inferred_types else None
